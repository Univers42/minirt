...
static t_vec3	refract_perp(const t_vec3 *uv, const t_vec3 *n, real_t ratio)
{
	t_vec3	neg_uv;
	real_t	cos_theta;
	t_vec3	n_scaled;
	...
	sum = vec3_add(uv, &n_scaled);
	return (vec3_mul_scalar(&sum, ratio));
}
...
t_vec3	vec3_refract(const t_vec3 *uv, const t_vec3 *n, real_t etai_over_etat)
{
	...
	r_out_perp = refract_perp(uv, n, etai_over_etat);
