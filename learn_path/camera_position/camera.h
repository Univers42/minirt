...
/*   Updated: 2026/01/03 15:00:25 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void camera_init(t_camera *camera, real_t aspect_ratio, int image_width)
{
	...
	/* Determine viewport dimensions using vfov */
	theta = degrees_to_radians(camera->vfov);
	h = (real_t)tan((double)theta / 2.0);
	viewport_height = (real_t)(2.0 * h * focal_length);
	...
	t_vec3 viewport_u_scaled = vec3_mul_scalar(&camera->u, viewport_width);
	/* Negate v to go downward in image space */
	t_vec3 neg_v = vec3_neg(&camera->v);
	t_vec3 viewport_v_scaled = vec3_mul_scalar(&neg_v, viewport_height);

	camera->pixel_delta_u = vec3_div_scalar(&viewport_u_scaled, (real_t)camera->image_width);
	camera->pixel_delta_v = vec3_div_scalar(&viewport_v_scaled, (real_t)camera->image_height);

	/* Calculate viewport upper left */
	t_vec3 focal_w = vec3_mul_scalar(&camera->w, focal_length);
	t_vec3 center_minus_focal = vec3_sub(&camera->center, &focal_w);
	t_vec3 half_u = vec3_div_scalar(&viewport_u_scaled, (real_t)2.0);
	t_vec3 half_v = vec3_div_scalar(&viewport_v_scaled, (real_t)2.0);
