...
/*   Updated: 2026/01/04 16:25:11 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* ray_color_depth with material emission and background support */
...
/* Render function: write PPM output to file in ../output/ directory */
static inline void camera_render(const t_camera *camera, FILE *out, const t_hittable_list *world)
{
	...
	char filename[256];
	snprintf(filename, sizeof(filename), "../output/final_render.ppm");
	...
	for (int j = 0; j < camera->image_height; ++j)
	{
		...
		for (int i = 0; i < camera->image_width; ++i)
		{
			...
			for (int sample = 0; sample < (int)camera->samples_per_pixel; ++sample)
			{
				...
				/* use camera-controlled max depth with background color */
