...
/*   Updated: 2026/01/03 14:50:35 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Lambertian (diffuse) material type */
...
/* Lambertian scatter implementation:
									  ...
									  const t_hit_record *rec, t_color *attenuation, t_ray *scattered)
{
	...
	/* construct scattered ray from hit point with new direction */
...
/* Metal scatter implementation with fuzz:
   reflected = reflect(r_in.direction(), rec.normal);
   reflected = unit_vector(reflected) + (fuzz * random_unit_vector());
   returns false if scattered ray points into surface */
								 ...
								 const t_hit_record *rec, t_color *attenuation, t_ray *scattered)
{
	...
	/* construct scattered ray from hit point with fuzzed direction */
	*scattered = ray_create(rec->p, fuzzed);
...
/* Dielectric scatter implementation:
   Check for total internal reflection or use Schlick's approximation.
   Always returns true (always scatters).
   Attenuation is always white (1.0, 1.0, 1.0). */
...
/* Lambertian constructor: allocate and initialize */
