...
/*   Updated: 2026/01/03 17:18:04 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Scene 1: Bouncing Spheres (original scene with random spheres) */
static int scene_bouncing_spheres(void)
{
...
/* Scene 2: Checkered Spheres */
static int scene_checkered_spheres(void)
{
	t_hittable_list world;
	t_material *materials[1024];
	int mat_count = 0;
	...
	t_texture *checker = checker_texture_create_colors(
		0.32,
		vec3_create(0.2, 0.3, 0.1),
		vec3_create(0.9, 0.9, 0.9));
	if (!checker)
	{
		fprintf(stderr, "Error: failed to allocate checker texture\n");
		cleanup_materials(materials, mat_count);
		return 1;
	}

	/* Create lambertian material with checker texture */
	t_material *checker_mat = lambertian_create_texture(checker);
	if (!checker_mat)
	{
		fprintf(stderr, "Error: failed to allocate checker material\n");
		checker->destroy(checker);
		free(checker);
		cleanup_materials(materials, mat_count);
		return 1;
	}
	materials[mat_count++] = checker_mat;

	/* Add two spheres with checker pattern */
	t_point3 lower_center = point3_create(0.0, -10.0, 0.0);
	t_sphere lower_sphere = create_sphere(&lower_center, 10.0, vec3_create(0.5, 0.5, 0.5), checker_mat);
	if (!hittable_list_add_sphere(&world, &lower_sphere))
	{
		fprintf(stderr, "Error: failed to add lower sphere\n");
		hittable_list_clear(&world);
		cleanup_materials(materials, mat_count);
		return 1;
	}
	...
	t_sphere upper_sphere = create_sphere(&upper_center, 10.0, vec3_create(0.5, 0.5, 0.5), checker_mat);
	if (!hittable_list_add_sphere(&world, &upper_sphere))
	{
		fprintf(stderr, "Error: failed to add upper sphere\n");
		hittable_list_clear(&world);
		cleanup_materials(materials, mat_count);
		return 1;
	}
...
int main(void)
{
	int scene = 2; /* Change this to select different scenes: 1 = bouncing spheres, 2 = checkered spheres */

	switch (scene)
	{
	case 1:
		return scene_bouncing_spheres();
	case 2:
