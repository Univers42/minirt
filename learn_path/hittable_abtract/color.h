...
/*   Updated: 2026/01/02 20:55:09 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int r = component_to_byte(pixel->e[X]);
	int g = component_to_byte(pixel->e[Y]);
	int b = component_to_byte(pixel->e[Z]);
	fprintf(out, "%d %d %d\n", r, g, b);
}
...
static inline t_vec3 ray_color_legacy(const t_ray *r)
{
	...
	real_t tt = 0.5 * (unit_dir.e[Y] + (real_t)1.0);
...
{
	t_hit_record rec;
	if (hittable_list_hit(world, r, (real_t)0.0, (real_t)INFINITY, &rec))
	{
	...
	t_vec3 unit_dir = unit_vector(&r->dir);
	real_t a = (real_t)0.5 * (unit_dir.e[Y] + (real_t)1.0);
	t_vec3 white = vec3_create(1.0, 1.0, 1.0);
	t_vec3 blue = vec3_create(0.5, 0.7, 1.0);
