...
/*   Updated: 2026/01/03 23:29:36 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Lambertian destructor: destroy owned texture ONLY if created via lambertian_create */
static inline void lambertian_destroy(t_material *mat)
{
	...
	/* Only destroy texture if it exists and mat->destroy is set to this function
	   (indicating we own it). For shared textures, don't destroy them here. */
	if (lamb->tex && lamb->tex->destroy)
	{
