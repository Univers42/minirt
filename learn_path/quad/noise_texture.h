...
/*   Updated: 2026/01/03 23:49:14 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline t_color noise_texture_value(const t_texture *tex, real_t u, real_t v, const t_point3 *p)
{
	...
	/* formula: 0.5*(1 + sin(scale * p.z + 10 * turb)) scaled by grey(0.5) */
	double angle = (double)(nt->scale * p->z) + 10.0 * (double)t;
	real_t s = (real_t)sin(angle);

	t_color base = vec3_create(0.5, 0.5, 0.5);
	t_color out = vec3_mul_scalar(&base, (real_t)(1.0 + s));
