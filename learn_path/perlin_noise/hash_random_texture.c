...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   hash_random_texture.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: dlesieur <dlesieur@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2026/01/03 01:21:08 by dlesieur          #+#    #+#             */
/*   Updated: 2026/01/03 22:38:22 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* Render four spheres with increasing noise mix (0% -> 100%) */
static int perlin_spheres_columns(void)
{
	/* create noise textures with varying mix */
	real_t scale = 4.0;
	int octaves = 5;
	t_color base = vec3_create(0.8, 0.3, 0.3); /* reddish base */

	real_t mixes[4] = {0.0, 0.33, 0.66, 1.0};

	t_material *mats[4];
	for (int i = 0; i < 4; ++i)
	{
		t_texture *nt = noise_texture_create(scale, mixes[i], octaves, base);
		if (!nt)
		{
			fprintf(stderr, "Error: failed to create noise texture (i=%d)\n", i);
			for (int k = 0; k < i; ++k)
				if (mats[k])
				{
	...
	/* place four spheres in a row (columns) with different noise mixes */
	...
	FILE *out = fopen("perlin_columns.ppm", "w");
	if (!out)
	{
	...
	printf("Rendering four spheres with noise mixes 0%%,33%%,66%%,100%%...\n");
	camera_render(&cam, out, &world);
	fclose(out);
	printf("Render complete! Output saved to perlin_columns.ppm\n");
