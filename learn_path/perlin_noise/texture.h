...
/*   Updated: 2026/01/03 23:01:42 by dlesieur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline t_texture *image_texture_create_png(const char *filename)
{
	...
	/* load and report failure */
	{
		...
		if (err)
		{
			/* lodepng may be disabled; print safe error message */
			fprintf(stderr, "lodepng error %u when loading %s\n", err, filename);
			lode_image_cleanup(&it->image);
			free(it);
			free(tex);
			return NULL;
...
static inline t_color noise_texture_value(const t_texture *tex, real_t u, real_t v, const t_point3 *p)
{
	...
	/* Scale point and compute turbulence */
	t_point3 sp = point3_create(p->x * nt->scale, p->y * nt->scale, p->z * nt->scale);
	real_t turb = perlin_turb(&nt->perlin, &sp, 7); /* 7 octaves */

	/* Map turbulence [-1,1] to [0,1] */
	real_t noise_val = (real_t)0.5 * ((real_t)1.0 + turb);

	/* Return grayscale noise */
	return vec3_create(noise_val, noise_val, noise_val);
}
...
/* Create noise texture with scale */
static inline t_texture *noise_texture_create(real_t scale)
{
	...
	nt->scale = (scale <= 0.0) ? 1.0 : scale;
	nt->perlin = perlin_create();
	tex->data = nt;
